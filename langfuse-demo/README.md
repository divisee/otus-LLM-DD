...
При этом такой трейс сложнее использовать для точечного анализа затрат по вложенным шагам: например, если нужно получить стоимость одной конкретной генерации, а не общую стоимость всего запуска.
