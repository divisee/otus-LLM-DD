...
## MLflow (GenAI / make_genai_metric)

### `genai-metrics-1.jpg` — общая информация по GenAI-эксперименту
Сводная вкладка MLflow по запуску GenAI-оценки (`run_genai_metric_experiment.py`): параметры запуска, список метрик и артефактов.

![](./genai-metrics-1.jpg)

### `genai-metrics-2.jpg` — графики с новой метрикой `genai-metric`
Графики/сравнения метрик, включая новую GenAI-метрику (кастомная оценка качества ответа).

![](./genai-metrics-2.jpg)

## Выводы и результаты эксперимента (по критериям проверки)

Краткое резюме по критериям оценки и фактические результаты запуска в MLflow.
...
- Grading prompt: 5 ставится только за полный/точный ответ, краткие или поверхностные ответы получают более низкий балл.
- Итоговая метрика агрегируется по датасету.
...
- Использована **Qwen/Qwen2.5-3B-Instruct**, которая поддерживает чат-формат (`messages` с ролями `user/system`) и совместима с `/v1/chat/completions`.
